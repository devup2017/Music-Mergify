...
-(void)baseInit
{
    ...
    if(self.frame.size.width != self.frame.size.height){
        ...
        float delta = ABS(self.frame.size.height)/2;
        if(self.frame.size.width > self.frame.size.height){
    ...
    //[self setFont:Regular(self.font.pointSize)];
    ...
    //self.endLabel.font = [UIFont fontWithName:@"Helvetica" size:8];
...
- (void)handlePanGestureRecognizer:(UIPanGestureRecognizer *)recognizer {
    ...
    if ([self.delegate respondsToSelector:@selector(progressBarGestureStateRecognizer:andEndDegree:)]) {
        
        [self.delegate progressBarGestureStateRecognizer:recognizer andEndDegree:angle];
    }
    
    
    if ([self.delegate respondsToSelector:@selector(progressBarEndDegree:)]) {
        
        [self.delegate progressBarEndDegree:angle];
    }
